...
describe('Array (defined)', () => {
  ...
  context('#splice', () => {
    ...
    it('should insert items using a positive start', () => {
      ...
      store.state.should.deep.equal([{ v: 7 }, { v: 2 }, { v: 4 }, { v: 8}, { v: 9 }, { v: 10 }, { v: 1 }, { v: 5 }, { v: 3 }]);
    });

    it('should insert items offset from the end using a negative start', () => {
      store.state = [{ v: 7 }, { v: 2 }, { v: 4 }, { v: 1 }, { v: 5 }, { v: 3 }];
      store.instance.splice(-2, 0, { v: 8}, { v: 9 }, { v: 10 });
      store.state.should.deep.equal([{ v: 7 }, { v: 2 }, { v: 4 }, { v: 1 }, { v: 8}, { v: 9 }, { v: 10 }, { v: 5 }, { v: 3 }]);
    });
  ...
  context('.length', () => {
    ...
    it('should extend the array when set to a higher value', () => {
      ...
      store.state.should.deep.equal([{ v: 1 }, { v: 2 }, { v: 3 }, { v: 4 }, { v: 5 }, { v: 6 }, { v: 7 }, { v: 0 }, { v: 0 }, { v: 0 }, { v: 0 }]);
    });
  });
  ...
  context('#set', () => {
    ...
    it('should fail for non-natural number properties', () => {
      store.state = [{ v: 1 }, { v: 2 }, { v: 3 }, { v: 4 }];
      expect(()=>store.instance.set(-10, {v:1})).to.throw();
      expect(()=>store.instance.set(undefined, {v:1})).to.throw();
      expect(()=>store.instance.set('missing', {v:1})).to.throw();
    });
  });
});
